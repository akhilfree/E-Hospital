...
export default function DoctorDash(){
  const { state } = useLocation();
    const username = state && state.doctor_name;
    console.log('Username',username)

    const[data, setData] = useState('')

    const thirdApiUrl = `http://127.0.0.1:8000/doctor-search/${username}`;
    ...
    fetch(thirdApiUrl)
      ...
      .then((data) => {
        ...
        setData(data[0]); // Store the first user data object in state
      })
      .catch((error) => {
